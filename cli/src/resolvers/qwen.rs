...
pub(super) fn resolve(dir: PathBuf, cfg: &ResolverConfig) -> crate::Resolution {
    let project_root = git_root(&dir, &fs_root(&dir, cfg));
    let root = project_root.clone().unwrap_or_else(|| dir.clone());
    let mut resolution = base(
        Agent::Qwen,
        dir.clone(),
        format!("git root: {}", root.display()),
        vec![
            "~/.qwen/QWEN.md + AGENTS.md".into(),
    ...
    if let Some(project_root) = project_root {
        let local = project_root.join(".qwen/QWEN.local.md");
        if exists(&local) {
